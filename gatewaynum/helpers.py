...
def bbauthentication_list_available(batch_id, dbx=None):
    ...
    try:
        ...
        q = """
            select cid.batchid, cid.callerid, cid.assigndate, cid.assignee, cid.wskey, cid.code_area, 
			cid.status, cid.releasedate, cid.blockdate, s.charearea as state
            from gw_BatchNumber bn 
			Left join GW_callerid cid
			ON bn.batch_id=cid.batchid
			LEFT JOIN GW_StateCode sc
			ON cid.code_area =sc.codearea
			LEFT JOIN GW_State s
			ON cid.state =s.id
            where bn.batch_id like ? and  cid.status =0 and  cid.wskey =0
            order by cid.callerid
            """
...
def bbauthentication_list_blocked(batch_id, dbx=None):
    ...
    try:
        ...
        q="""
            select cid.batchid, cid.callerid, cid.assigndate, cid.assignee, cid.wskey, cid.code_area, 
			cid.status, cid.releasedate, cid.blockdate, cid.blockperiod, s.charearea as state, gbn.est_releasedate, DATEDIFF(day,  GETDATE(), gbn.est_releasedate) AS DateDiff
            from gw_BatchNumber bn 
			Left join GW_callerid cid
			ON bn.batch_id=cid.batchid
			LEFT JOIN GW_StateCode sc
			...
			LEFT JOIN GW_State s
			ON cid.state =s.id
			LEFT Join gw_block_num gbn
			on cid.callerid=gbn.callerid
            where bn.batch_id like ? and cid.status =2 
            order by cid.callerid
        ...
        # q = """
        #     select cid.batchid, cid.callerid, cid.assigndate, cid.assignee, cid.wskey, cid.code_area, 
		# 	cid.status, cid.releasedate, cid.blockdate, cid.blockperiod, s.charearea as state
        #     from gw_BatchNumber bn 
		# 	Left join GW_callerid cid
		# 	ON bn.batch_id=cid.batchid
		# 	LEFT JOIN GW_StateCode sc
        ...
        for r in rows:
            ...
            o.releasedate =r.releasedate 
            o.blockdate =r.blockdate 
            o.blockperiod =r.blockperiod 

            o.state =r.state 
            o.wskey = r.wskey
            o.code_area = r.code_area
            o.assignee = r.assignee
            o.est_releasedate=r.est_releasedate
            o.DateDiff = r.DateDiff
...
def bbauthentication_list_usedbyaccntid(batch_id, dbx=None):
    l = []
    db = dbx
    # print("running here")
    try:
        ...
        q = """
			...
			LEFT JOIN GW_State s
			ON cid.state =s.id
            where bn.batch_id like ? and  cid.status in (1) and cid.wskey!=0 
            order by cid.callerid
            """
        print(q)
        rows = db.cur.execute(q,'%{0}%'.format(batch_id)).fetchall()
...
def bbauthentication_list_testing(batch_id, dbx=None):
    ...
    try:
        ...
        q = """
			...
			LEFT JOIN GW_State s
			ON cid.state =s.id
            where bn.batch_id like ? and  cid.status =3 
            order by cid.callerid
            """
...
def bbauthentication_list_reserve(batch_id, dbx=None):
    l = []
    db = dbx
    # print("running here")
    # 0 available
    ...
    try:
        ...
        q = """
            select cid.batchid, cid.callerid, cid.assigndate, cid.assignee, cid.wskey, cid.code_area, am.name, rn.reqdate, rn.remark,
			cid.status, cid.releasedate, cid.blockdate, s.charearea as state
            from gw_BatchNumber bn 
			Left join GW_callerid cid
			ON bn.batch_id=cid.batchid
			...
			LEFT JOIN GW_State s
			ON cid.state =s.id
			LEFT JOIN GW_reserve_num rn
			ON cid.callerid =rn.callerid
			LEFT JOIN GW_AccountManager am
			ON rn.acctmanager =am.id
            where  bn.batch_id like ? and cid.status =5 
            order by cid.callerid
            """
...
def bbauthentication_extend(idxlist, accountid, user, pwd):
    le = []
    db = None
    ...
    try:
        ...
        for dic in idxlist:
            ex = bbauthentication_extblock_callerid(db, dic['callerid'], accountid,user, pwd)
            print(dic)
...
def bbauthentication_setusednum(idxlist, accountid, user, pwd):
    le = []
    db = None
    ...
    try:
        ...
        for dic in idxlist:
            ex = bbauthentication_setused_callerid(db, dic['callerid'], accountid,user, pwd)
            print(dic)
...
def bbauthentication_suspend_callerid(dbx, callerid, suspend=2):
    ...
    try:
        ...
        insert_into_blocked_record_log(batchid, callerid, wskey, code_area,status, user)
        insert_into_blocked_record(callerid, wskey, status, user)
        
        q = """
            update GW_callerid set status = ?, blockperiod='6' where callerid = ? 
            """
        print("callerid block")
...
def bbauthentication_block_callerid(dbx, callerid, accountid,user, pwd):
    ...
    try:
        ...
        insert_into_blocked_record_log(batchid, callerid, wskey, code_area,2, user)
        insert_into_blocked_record(callerid, wskey, 2, user)


        q = """
            update GW_callerid set status = 2, blockdate=?,blockperiod='6' where callerid = ? 
            """
        print(callerid)
...
def bbauthentication_resume_callerid(dbx, callerid, accountid,user, pwd):
    ...
    try:
        ...
        insert_into_blocked_record_log(batchid, callerid, wskey, code_area,0, user)
        # delete
        delete_blockcallerid(callerid, 2, user, pwd)
...
def bbauthentication_extblock_callerid(dbx, callerid, accountid,user, pwd):
    ex = None
    db = None
    print("set number to ext block")
    try:
        # db = initdb(dbx)
        db = initdb(dbx=None, autocommit=False, user=user, pwd=pwd)
        ...
        q = """
            UPDATE GW_callerid SET blockperiod=? WHERE callerid = ?
           """

        print(callerid)

        db.cur.execute(q, 12,callerid)

        x= get_lastblockdate(callerid)
        est_releasedate = x + relativedelta(months=+6)

        qx = """
            update GW_block_num set est_releasedate = ? where callerid = ? 
            """
        print(est_releasedate)
        db.cur.execute(qx, est_releasedate, callerid)
...
def bbauthentication_setused_callerid(dbx, callerid, accountid,user, pwd):
    ex = None
    db = None
    print("set number as used")
    ...
    try:
        ...
        q = """
            update GW_callerid set status = 1 where callerid = ? 
            """
        print(callerid)
        db.cur.execute(q, callerid)

        qs = """
            delete from GW_reserve_num
            where callerid = ?
            """
        db.cur.execute(qs, callerid)
...
def bbauthentication_update_sippstn(callerid, pstn,  username):
    db = None
    ...
    try:
        ...
        # insert_into_sql_log(qs, accountid, username, db, -1)
        qs = "UPDATE GW_callerid SET block_period={0} WHERE callerid='{1}'".format(pstn, callerid)
...
def authority_submit(o):
    ...
    try:
        ...
        q = """
            select sloginpassword from GNlogin 
            where sloginname = ?
            """
...
def supervisor_list(dbx=None):
    ...
    try:
        ...
        q = """
            select sloginname, ip from GNlogin 
...
def userdetail_load(accountid,gLogin_Wholesaler):
    ...
    try:
        ...
        dic['testnum'] = userdetail_count_test(accountid, db)
        dic['reservenum'] = userdetail_count_reserve(accountid, db)
        dic['assignednum'] = userdetail_count_assign_ws(accountid, db)
        ...
        dic['rt015usedbyaccntid'] = bbauthentication_list_usedbyaccntid(accountid, db)

        dic['rt015block'] = bbauthentication_list_blocked(accountid, db)
        dic['rt015test'] = bbauthentication_list_testing(accountid, db)
        dic['rt015reserve'] = bbauthentication_list_reserve(accountid, db)
...
def userdetail_count_available(batch_id, dbx=None):
    ...
    try:
        ...
        q = """
            ...
            where bn.batch_id=cid.batchid and bn.batch_id like ? and  cid.status = 0 and cid.wskey = 0
            """
...
def userdetail_count_used(batch_id, dbx=None):
    ...
    try:
        ...
        q = """
            ...
            where bn.batch_id=cid.batchid and bn.batch_id like ? and  cid.status in (1) and cid.wskey!=0
            """
        r = db.cur.execute(q, batch_id).fetchone()
        if r is not None:
            o = r.usednum
...
def userdetail_count_assign_ws(batch_id, dbx=None):
    o = 0
    db = dbx
    ...
    try:
        ...
        q = """
            select count(cid.status) as usednum from gw_BatchNumber bn, GW_callerid cid
            where bn.batch_id=cid.batchid and bn.batch_id like ? and  cid.status in (0) and cid.wskey!=0
            """
        r = db.cur.execute(q, batch_id).fetchone()
        if r is not None:
            o = r.usednum
...
def userdetail_count_test(batch_id, dbx=None):
    o = 0
    db = dbx
    ...
    try:
        ...
        q = """
            select count(cid.status) as testnum from gw_BatchNumber bn, GW_callerid cid
...
def userdetail_count_reserve(batch_id, dbx=None):
    o = 0
    db = dbx
    ...
    try:
        ...
        q = """
            select count(cid.status) as reservenum from gw_BatchNumber bn, GW_callerid cid
            where bn.batch_id=cid.batchid and bn.batch_id like ? and  cid.status = 5
            """
        r = db.cur.execute(q, batch_id).fetchone()
        if r is not None:
            o = r.reservenum
    ...
    try:
        ...
        if keyword == '':
            q = """
                ...
                LEFT JOIN GW_State s
                ON cid.state =s.id
                order by cid.batchid, cid.CallerID
            ...
            q = """
                ...
                LEFT JOIN GW_State s
                ON cid.state =s.id
                where cid.callerid like ? or cid.batchid like ? or cid.state like ?
                order by cid.batchid, cid.CallerID
            """
            print(keyword)
...
def tlogin_password_get(username):
    ...
    try:
        ...
        q = """
            select sloginname, sloginpassword from GNlogin 
            where sloginname = ?
            """
...
def tlogin_get(username, password):
    ...
    try:
        ...
        q = """
            ...
            functionflag, department, admin, expiry from GNlogin 
            where sloginname = ?
            """
        r = db.cur.execute(q, username).fetchone()
        if r is None:
...
def changepassword(o):
    ...
    try:
        ...
        q = 'update GNlogin set sloginpassword = ?, expiry = ? where sloginname = ?'
...
def view015numbers_excel_list(wskey):
    ...
    try:
        ...
        q = """
			...
			ON cid.state =s.id
            where  cid.wskey = ? 
            order by cid.batchid, cid.assigndate        
            """
        ...
        for r in rows:
            if r.status==3:
                r.status='3-Test Number'
            elif r.status==2:
                r.status='2-Blocked Number'
            elif r.status==1:
                r.status='1-Used Number'
            else:
                r.status=r.status
		...
		# 	LEFT JOIN GW_State s
		# 	ON cid.state =s.id
        #     where cid.status!=0 and cid.wskey = ? 
        #     order by cid.batchid, cid.assigndate        
        #     """
...
def view015numbers_excel_list_batchid(batchid):
    ...
    try:
        ...
        q = """
			...
			LEFT JOIN GW_State s
			ON cid.state =s.id
            where cid.status=0 and cid.wskey=0 and cid.batchid like ?
            order by cid.batchid, cid.assigndate        
            ...
            # where status=0 and wskey=0 and batchid like ? 
            # order by assigndate 
        ...
        q = """
			...
			LEFT JOIN GW_State s
			ON cid.state =s.id
            where cid.wskey!=0 and cid.batchid like ?
            order by cid.batchid, cid.assigndate        
            
            """

            # select batchid, callerid, status, wskey, assigndate, blockdate, releasedate, code_area, state from GW_callerid
            # where wskey!=0 and batchid like ? 
            # order by assigndate 
...
def availablereserved_count(sfrom, sto, dbx=None):
    x = 0
    db = dbx
    ...
    try:
        ...
        q = """
            select count(callerid) as cnt from gw_callerid 
            where wskey not in (0) and status = 0 
            and callerid between ? and ?
            """
        r = db.cur.execute(q, sfrom, sto).fetchone()
        if r is not None:
...
def get_wskey_reserve(sfrom, dbx=None):
    ws = 0
    db = dbx
    ...
    try:
        ...
        q = """
            select wskey from gw_callerid 
            where 
            callerid =?
            """
        r = db.cur.execute(q, sfrom).fetchone()
        if r is not None:
            ws = r.wskey
            # print(x)
...
def get_wskey_reserve_count(sfrom, sto, dbx=None):
    cws = 0
    db = dbx
...
def num_tiedmanager_count(sfrom, sto, dbx=None):
    x = 0
    db = dbx
    ...
    try:
        ...
        q = """
            select count(callerid) as cnt from gw_reserve_num
    ...
    try:
        ...
        if x != expectedcount:
            ...
            if y > 0:
                ...
                raise UIException('numbers are currently in use')
        ...
        #         q = """
        #             insert into bb_authentication (bb_rt015, bb_creationdate, bb_allowpstn, bb_status, bb_nicenum, 
        #             bb_wskey, bb_displayname, bb_prepaid) 
        #             values (?, current_timestamp, 0, 0, 0, ?, ?, 0)
        #             """
        #         db.cur.execute(q, num, wskey, num)
        #         n += 1
...
def reserved015(sfrom, sto, expectedcount,  accntid, remark, user, pwd):
    x = 0
    y = 0
    db = None
    ...
    try:
        ...
        x = availablereserved_count(fromstr, tostr, db)

        cws = get_wskey_reserve_count(fromstr, tostr, db)
        # 
        print(cws)
        if x != expectedcount:
            y = num_tiedmanager_count(fromstr, tostr, db)

            if y > 0:
                raise UIException('{0} numbers are currently in use'.format(y))
            elif x >= 0:
                raise UIException('Unable to process.There are some number not assign to [Wskey] or Total Number not match')
 
        print(x)

        if x == expectedcount: #and cws== expectedcount:
            
            print("here xx")
            q = """
                update gw_callerid set status = 5
                where callerid between ? and ?
               """
            ...
            for i in range(num1, num2 + 1):
                ...
                ws = get_wskey_reserve(num, db)

                q = """
                    insert into gw_reserve_num (callerid, reqdate, wskey, status, acctmanager, remark, [user]) 
                    values (?, current_timestamp, ?, 5, ?, ?,?)
                    """
                print(user)
                # db.cur.execute(q, num,id,user)
                db.cur.execute(q, num, ws, accntid, remark,user)
            ...
            if n == expectedcount:
                ...
                raise Exception('Failed to reserve numbers {0} - {1} for Accnt Manger {2}'.format(fromstr, tostr, accntid))
...
def unassign_wskey(sfrom, sto, user, pwd):
    ...
    try:
        ...
        q = """
            update gw_callerid set wskey = 0 
            where callerid between ? and ?
            """
        db.cur.execute(q, fromstr, tostr)
...
def unreserved015(sfrom, sto, user, pwd):
    db = None
    ...
    try:
        ...
        q = """
            update gw_callerid set status = 0 
            where callerid between ? and ?
            """
        db.cur.execute(q, fromstr, tostr)
        ...
        qs = """
            ...
            where callerid between ? and ?
            """
        db.cur.execute(qs, fromstr, tostr)
...
def upload015(batch_id, sfrom, sto, expectedcount,  user, pwd):
    ...
    try:
        ...
        if int(x) == int(expectedcount) and y == 0:
            print(fromstr)
            num1 = int(fromstr)
            # print(num1)
            num2 = int(tostr)
...
def manager_list(dbx=None):
    ...
    try:
        ...
        q = "select id, name, position, company from GW_AccountManager order by id desc"
        # print(q)
        rows = db.cur.execute(q).fetchall()
        for r in rows:
            o = models.Manager()
            o.id = r.id
            o.name = utils.decode(r.name)
...
def list_subarea(dbx=None):
    ...
    try:
        ...
        q = "select id, codearea, charearea from gw_state order by id desc"
        print(q)
        rows = db.cur.execute(q).fetchall()
        for r in rows:
            o = models.statearea()
...
def chargearea_add(codearea,charearea,user):
    ...
    # return y
def manager_add(name, position, company):
    db = None
    ...
    try:
        ...
        q = """
            insert into GW_AccountManager(name, position, company)
            values (?, ?, ?)
            """

        db.cur.execute(q, str(name), str(position), str(company))
        db.commit()
...
def insert_into_blocked_record_log(batchid, callerid, wskey, code_area,status, user): 
    db = None
    try:
        db = initdb()
...
def insert_into_blocked_record(callerid, wskey, status, user): 
    db = None
    try:
        db = initdb()

        q = """
            insert into GW_block_num (callerid, reqdate, wskey, status, est_releasedate, [user]) 
            values (?, ?, ?, ?, ?,?)
            """
        print(q)
        est_releasedate = datetime.today() + relativedelta(months=+6)

        db.cur.execute(q, callerid,  datetime.now(), wskey, status, est_releasedate, user)
        db.commit()
...
def delete_blockcallerid(callerid,status, user, pwd):
    db = None
    
    try:
        db = initdb(dbx=None, autocommit=False, user=user, pwd=pwd)
        
        q = 'delete from GW_block_num where callerid = ? and status=2'
        db.cur.execute(q, callerid)
        db.commit()
...
def get_lastblockdate(callerid,  dbx=None):
    x = 0
    db = dbx
    print("extend")
    try:
        ...
        q = """
            select est_releasedate from GW_block_num where callerid = ?
            """
        r = db.cur.execute(q, callerid).fetchone()
        if r is not None:
            x = r.est_releasedate
