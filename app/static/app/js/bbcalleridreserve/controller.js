function BBCalleridreserveCtrl($scope, $http, $modal) {
  ...
  $scope.updatePstn = function ($data) {
    var s = $data == true ? 12 : 6;

    if (s == $scope.editpstn.data.bb_allowpstn)
    return false;
  ...
  $scope.unreservenumber_ = function () {
    var listreserved = $scope.listreserved;
    var lx = _.filter(listreserved, function (o) {
      return o.selected == true && o.status == 5
    });
    var ids = _.map(lx, function (o) {
      return { callerid: o.callerid};
    });
    ...
    $http.post(route.bbcallerid.unreservenumber, o).success(function (data) {
      if (data.error == 1) {
        toastr.error(data.message);
      }
  ...
  $scope.unreservenumber = function () {
    if ($scope.selected.count < 1)
    return;

    bootbox.confirm("Assign to use numbers?", function (result) {
      if (result == true) {
        $scope.unreservenumber_();
      }
    });
  }
...
app.controller('BBCalleridreserveCtrl', ['$scope', '$http', '$modal', BBCalleridreserveCtrl]);
